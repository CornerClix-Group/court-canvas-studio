// Client-side email template generator for preview purposes
// Mirrors the logic from send-estimate-email/index.ts
...
export function getEstimateEmailSubject(estimateNumber: string): string {
  return `Your Estimate from CourtPro Augusta - ${estimateNumber}`;
}

// Lump Sum Email - Marketing focused with bullet points
function generateLumpSumEmailHTML(estimate: EstimateForEmail, scopeBullets: ScopeBullet[], hasAttachments: boolean): string {
...
function generateDetailedScopeEmailHTML(estimate: EstimateForEmail, lineItems: LineItem[], hasAttachments: boolean): string {
  const customerName = estimate.customer?.contact_name || "Valued Customer";
  const validUntil = estimate.valid_until ? formatDate(estimate.valid_until) : "30 days from receipt";
      ...
      <table width="100%" cellpadding="0" cellspacing="0" style="max-width: 600px; margin: 0 auto; background-color: #ffffff;">
        ...
        <tr>
          <td style="padding: 30px 30px 20px 30px;">
            <h2 style="color: #1f2937; margin: 0 0 15px 0; font-size: 22px;">Hello ${customerName},</h2>
            <p style="color: #4b5563; margin: 0; font-size: 16px; line-height: 1.6;">
              Thank you for your interest in our court construction services. Please find your detailed estimate below.
            </p>
          </td>
        </tr>
        ...
        <tr>
          <td style="padding: 0 30px 20px 30px;">
            <h3 style="color: #1f2937; margin: 0 0 15px 0; font-size: 18px; border-bottom: 2px solid #059669; padding-bottom: 10px;">Scope of Work</h3>
            <table width="100%" cellpadding="0" cellspacing="0" style="border: 1px solid #e5e7eb; border-radius: 8px; overflow: hidden;">
              <tr style="background-color: #1e3a5f;">
                <td style="padding: 12px; color: #ffffff; font-weight: 600; font-size: 14px;">Description</td>
                <td style="padding: 12px; color: #ffffff; font-weight: 600; font-size: 14px; text-align: right; width: 100px;">Amount</td>
              </tr>
              ${itemsHTML}
              <tr style="background-color: #f8fafc;">
                <td style="padding: 15px; font-weight: 700; font-size: 16px; color: #1f2937;">Total</td>
                <td style="padding: 15px; font-weight: 700; font-size: 18px; text-align: right; color: #059669;">${formatCurrency(estimate.total)}</td>
              </tr>
            </table>
          </td>
        </tr>
...
// Main function that chooses the right template based on display_format
export function generateEstimateEmailHTML(
  ...
  // Use lump_sum format if specified, otherwise use detailed scope
  if (estimate.display_format === 'lump_sum') {
    return generateLumpSumEmailHTML(estimate, scopeBullets, hasAttachments);
  }
  return generateDetailedScopeEmailHTML(estimate, lineItems, hasAttachments);
}
